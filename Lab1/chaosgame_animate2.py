...
"""
def sk_midpoint (point1, point2):
    sk_midpoint = [ 0.5 * (point1[0] + point2[0]) , 0.5 * (point1[1] + point2[1])]
    return sk_midpoint
...
for i in range(1,N-1):
    ...
    newxy = sk_midpoint((x[i-1],y[i-1]),i_corner)
    x[i] = newxy[0]
    y[i] = newxy[1]
...
"""
def sk_midpoint (point1, point2):
    sk_midpoint = [ 0.3 *point1[0] + 0.7 * point2[0] , 0.3 * point1[1] + 0.7 * point2[1]]
    return sk_midpoint


fig = plt.figure()
ax = plt.axes(xlim=(-0.1, 1.1), ylim=(-0.1, 1))
dots, = ax.plot([], [], 'bo', markersize = 1)
